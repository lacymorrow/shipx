...
- **Steps delegated to CI are actually checked** (`steps/ci.ts`, `steps.watchCi`, `--no-watch-ci`). When `githubRelease` or `homebrew` is set to `false`, shipx used to print `CI-handled (via tag push)` and exit 0 without ever looking at CI — a release whose tag-triggered workflow failed outright was reported as clean. shipx now watches the runs the pushed tags kicked off via `gh run list`, prints each run's workflow, tag, result, and duration, and exits non-zero when one failed. A pushed tag that triggers no run at all gets a loud warning (the misconfigured-trigger case), runs still going after the five-minute cap are reported as pending with their URLs, and Ctrl-C during the watch exits clean since the release itself is already done. On by default when `gh` is available and at least one step is delegated. ([#59](https://github.com/lacymorrow/shipx/issues/59))
- **`--multi` releases watch their delegated CI runs too** (`multi.ts`). Batch mode pushed tags per project during the prepare phase and reported success from the local steps alone — the watcher from #59 only covered single-project releases. Multi mode now runs a final phase that watches the tag-triggered runs of every project whose config delegates `githubRelease` or `homebrew` to CI, marks each project in the batch summary (failed, still running, or no run appeared for the tag), and exits non-zero when a watched run failed. `steps.watchCi` and `--no-watch-ci` mean the same thing they do in single mode. ([#62](https://github.com/lacymorrow/shipx/issues/62))
