...
- **`shipx.config.mts` and `shipx.config.mjs`** (`config.ts`). Lookup order is now `.mts`, `.ts`, `.mjs`, `.js`. `.mts` is the recommended config file because it's ESM regardless of the project's `package.json` `type`.

### Fixed

- **No Node module-type warning when loading a config** (`config.ts`). Running shipx under Node in a project without `"type": "module"` printed `MODULE_TYPELESS_PACKAGE_JSON` for `shipx.config.ts`. shipx now drops that one warning while importing the config, so ESM and CommonJS projects both load quietly.
